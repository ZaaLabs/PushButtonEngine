	...
	internal class Entity extends PBObject implements IEntity, IEventDispatcher
    {
        ...
        public function deserialize(xml:XML, registerComponents:Boolean = true):void
        {
            ...
            for each (var componentXML:XML in xml.*)
            {
                ...
                if (componentClassName.length > 0)
                {
					...
					var type:Class = TypeUtility.getClassFromName(componentClassName);
					if(!type)
					{
						Logger.error(this, "deserialize", "Unable to find type '" + componentClassName + "' for component '" + componentName +"' on entity '" + name + "'.");
						continue;
					}
            ...
            if (setsAttr)
            {
                ...
                if (setNames)
                {
                    ...
                    {
                        var pbset:PBSet = context.lookup(thisName) as PBSet;
                        if (!pbset) 
                        {
        ...
        public function addComponent(component:IEntityComponent, componentName:String):Boolean
        {
			...
			if(component.context != context)
				throw new Error("Component and entity are not from same context!");
