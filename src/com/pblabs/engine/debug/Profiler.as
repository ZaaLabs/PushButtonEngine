...
package com.pblabs.engine.debug
{
    ...
    public class Profiler
    {
        ...
        public static var initializedKeyboard:Boolean = false;
        public static var keyboardHotKeyDown:Boolean = false;
        
        /**
         * General profiler initialization; this is what sets up the profiler hotkey. 
         */
        public static function initialize(stage:Stage):void
        {
            if(!initializedKeyboard)
            {
                stage.addEventListener(KeyboardEvent.KEY_DOWN, onKeyDown);
                stage.addEventListener(KeyboardEvent.KEY_UP, onKeyUp);
                initializedKeyboard = true;
            }
        }
        
        protected static function onKeyDown(e:KeyboardEvent):void
        {
            if(e.keyCode == InputKey.P.keyCode)
                keyboardHotKeyDown = true;
        }
        
        protected static function onKeyUp(e:KeyboardEvent):void
        {
            if(e.keyCode == InputKey.P.keyCode)
                keyboardHotKeyDown = false;
        }
        ...
        public static function enter(blockName:String):void
        {
            ...
            if(_stackDepth == 0)
            {
                // If they press, then release the hotkey, start/stop and dump
                // the profiler.
                if (keyboardHotKeyDown)
                {
                    if(!enabled)
                    {
                        _wantWipe = true;
